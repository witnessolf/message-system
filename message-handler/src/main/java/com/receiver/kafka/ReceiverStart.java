...
public class ReceiverStart {
    ...
    /**
     * 每一个Receiver初始化的时候做了动态的切面，拿到对应的@KafkaListener注解，修改其groupId。
     * @return
     */
    @Bean
    ...
    /**
     * producer 将tag写进header里, receiver根据header里的tag过滤
     * @return
     */
    @Bean
            ...
            if (Optional.ofNullable(consumerRecord.value()).isPresent()) {
                for (Header header : consumerRecord.headers()) {
                    if (header.key().equals(tagIdKey)
                            && new String(header.value()).equals(new String(tagIdValue.getBytes(StandardCharsets.UTF_8)))) {
                        return false;
                    }
                }
            }
